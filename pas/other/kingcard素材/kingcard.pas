program kingcard;{最新bug未修正：异地存盘有限读取次数c:\windows\system32\savebackKCn.dll。多天候利率存储=。=。竞赛模式电脑多带点钱212（是否需要需测试）。电脑输了不跟。本人不跟。新游戏模式的声音。银行利率总收入。}
type
 cardtype=record
  a:char;
  b:string;
...
begin
 ...
 begin
  ...
  writeln('现在你的还款期限为              ',bankm[7]);writeln;
  writeln('您的总净资产额为                ',money[0]+bankm[1]-bankm[6]);
 end;
 writeln('********************************************************************************');
end;
procedure writehelp;
...
begin
 ...
 writeln('6& 最终额外奖励为赌局税收的20%和额外的经验值（很少为：税收总数*0.2*0.002；很多为：税收总数*0.4*0.005）!');
 readln;
 writeln('7& 游戏的单场赔率为：');
 for i:=1 to 10 do
...
if testtype then begin
 ...
 writeln(f1);
 writeln(f1,'等级：                ',lv);
 writeln(f1,'总资产(计算存款)：    ',money[0]+bankm[1]-bankm[6]);
 writeln(f1,'********************************************************************************');
 write(f1,'单场玩家数目':12);
 for i:=2 to 5 do
  write(f1,cc[i]:15);
...
begin
 ...
 if s='-loser' then begin loser:=true;writeln('你选择了向对手投降！将在本局结束后自动离开本赌桌！'); end;
end;
procedure readlntmp;
var
 s:string;
...
// bankm[5]:=trunc((exp*trunc(exp/1000)*trunc(exp/1000)*trunc(exp/1000)-bankm[6])/(1+min(4,trunc(exp/1000))))
 ...
 if s=9 then begin writeln('您已经离开银行！您想要再次进入银行(-bank)还是加入赌桌呢(人数范围2-5)？');savegame;firstbank:=true;exit; end else
 ...
 if s=5 then begin write('请输入您要贷款的金额：          ');readln(nt);val(nt,n,code);if (n<=0)or(code<>0) then begin writeln('错误的输入格式！');readlntmp;bank;exit; end;
   if n<=bankm[5] then begin inc(money[0],n);inc(bankm[6],n*(1+ max(0,min(9,trunc(bankm[2]/10))) ));writeln('您贷到了',n,'。您的现金余额为',money[0],'。您的贷款额为',bankm[6]:10,'。');write('   您的还款期限为',bankm[7],'天！');readlntmp;bank;exit;
...
begin
 ...
 write('游戏版本号V2.110212  增加新的游戏模式和称号。电脑死亡后不会偿还额外赌债。上调1级最高赌注。缩短还款期限。任何输入框均可输入命令。修正声音及文字显示细节。');
 readln;
 newload;
 deadman;
      ...
      begin
       ...
       begin
        ...
        case playtype of
         ...
         1:begin
            maxtpt1:=trunc(0.05*money[0]);
            if maxtpt1<100 then maxtpt1:=100;
            for i:=0 to players-1 do
             if money[i]<maxtpt1 then maxtpt1:=money[i];
            writeln('最低赌注根据现有金钱的 5% 确定为               ：',maxtpt1);
            for i:=0 to players-1 do
             if money[i]<mint then mint:=money[i];
            write('最高赌注根据赌桌上最穷的人确定为               ：',mint);
            if mint=money[0] then writeln('（ALL IN）') else writeln;
            writeln('您目前的等级为                                 ：',lv);
           end;
        end;
        writeln('请输入您最终选择的赌注：');
...
//*****************************************change money**********************************************
  for i:=0 to players-1 do begin
   writeln(namep[i]:namek,'':14,'             携带的金钱为：',tmoney[i]:11);
   ...
   if namep[i]='$大财主范威$' then begin
    ...
    if money[0]+bankm[1]<=0 then writeln('（死亡）')
      else if money[0]<=0 then writeln('（破产）') else writeln;
   end;
  end;
  if playtype=0 then begin
    writeln;
    writeln('':namek,'':14,'         最初携带的金钱为：',tmoney[0]+mid+midtmp:11);
    writeln('               本局最终收益：',(money[0]-tmoney[0]-mid-midtmp):11,'     金钱变为：',money[0]:11);
  end;
