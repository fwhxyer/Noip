...
var
 ...
 goto00,DbTforH,onlyf12:boolean;
//********************************************main************************************************************
...
{      for j:=0 to 12 do  //速度太慢！
    ...
    const
     cdetail:parr=(
             ...
             (4573800,4573800,4538160,4502520,4466880,4472820,4472820,4472820,4472820,4466880,4502520,4538160,4573800),
...
begin
  ...
  rewrite(f3);
  writeln(f3,'( //以下为winphash的全部数据');for imost:=1 to 3 do writeln(f3);
  for players:=1 to 8 do begin
  {}if players=1 then onlyf12:=true else onlyf12:=false; {---------if-onlyf12------------}
    if onlyf12 then writeln(f1,'(');
    if onlyf12 then writeln(f2,'(');
    writeln(f3,'(  //以下为players',players,'时的winphash数据');
    for imost:=1 to 13 do begin
      if onlyf12 then writeln(f1,'(   //',imost,'-------------------------------------------------------------------------');
      if onlyf12 then writeln(f2,'(   //',imost,'-------------------------------------------------------------------------');
      writeln(f3,'(   //',imost,'-------------------------------------------------------------------------');
      for j:=1 to 13 do begin
        if onlyf12 then write(f1,'(  ');
        if onlyf12 then write(f2,'(  ');
        write(f3,'(  ');
        for k:=1 to 2 do begin
          if k=1 then begin if onlyf12 then write(f1,'( ') end else begin if onlyf12 then write(f1,'   ( ') end;
          if k=1 then begin if onlyf12 then write(f2,'( ') end else begin if onlyf12 then write(f2,'   ( ') end;
...
//          if not true then //本行是打格式表 上面一行是打正式数据表
          begin
          ...
          for l:=0 to 9 do begin
           {if l<>9 then begin
              if onlyf12 then write(f1,'P,')
            end else begin
              if onlyf12 then write(f1,'P');
            end; 这里是格式！}
            if l<>9 then begin
              if onlyf12 then write(f1,p[l],',')
            end else begin
              if onlyf12 then write(f1,p[l]);
            end;
            if l<>9 then begin
              if onlyf12 then write(f2,ph[l],',')
            end else begin
              if onlyf12 then write(f2,ph[l]);
            end;
    ...
    if onlyf12 then writeln(f1,');');
    if onlyf12 then writeln(f2,');');
    if players<>8 then writeln(f3,'),  //以上为players',players,'时的winphash数据') else writeln(f3,')   //以上为players8时的winphash数据');
  end;
  writeln(f3,'); //以上为winphash的全部数据');
  close(f1);
  close(f2);
  close(f3);
end.
