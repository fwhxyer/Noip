...
writeln('　　　　◥███████◤  　　    宝      　　◥███████◤  ');
writeln('　　　　　　◢███◣ 　           !!        　　　◢███◣ ');
if sound then for i:=1 to 12 do write(chr(7)) else for i:=1 to 1 do write(chr(7));
end;
procedure writehis;
const
...
('###########')));
      ...
      begin
       ...
       if s='zhizunbao' then begin s:='1 2 3 4 5';zhizunbaobo:=true;fillchar(bool,sizeof(bool),0);end;
       while not boolt do
       begin
        writeln('您输入了错误的格式，请按照正确格式输入！');
        readln(s);ao(s);
        if s='zhizunbao' then
        begin s:='1 2 3 4 5';zhizunbaobo:=true;fillchar(bool,sizeof(bool),0);if boolt then break; end;
       end;
       if asdf then exit;
       for i:=1 to (length(s)+1)div 2 do
       begin
