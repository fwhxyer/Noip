...
begin
 ...
 write('游戏版本号V2.110212  增加了新的游戏模式。增加了胜利称号。电脑死亡后不会偿还额外赌债。上调了1级赌注。缩短了高等级的还款期限。现在任何输入框均可输入命令。');
 readlntmp;
 newload;
 deadman;
...
procedure game;{&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&game&&&&&&&&&&&&&&&&&&&&}
var
 i,j,tmp,maxtpt1,winnumpt0,winnumpt1:integer;mid,mtmp,shui,mint:int64;
 getout:boolean;
...
//*****************************************change money**********************************************
  for i:=0 to players-1 do begin
   ...
   write('                       收益：',(money[i]-tmoney[i]):11,' 金钱变为：',money[i]);
   if namep[i]='$大财主范威$' then begin
    if money[i]<-(255555+1000*lv) then writeln('（死亡）') else writeln;
   end else begin
    if money[i]<-(5000+800*lv) then writeln('（死亡）') else writeln;
   end;
  end;
