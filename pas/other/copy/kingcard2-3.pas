program kingcard;{最新bug未修正：异地存盘有限读取次数c:\windows\system32\savebackKCn.dll。多天候存储=。=}
type
 cardtype=record
  a:char;
  b:string;
...
var
 ...
 sound,firstbank,loser,notloser,firstdead,testtype:boolean;
...
begin
 ...
 writeln('4& 本游戏在任何有效输入框中均支持六条命令语句输入（1.(-he)）（2.(-sa)）（3.获取历史记录(-hi)）（4.保存当前的历史记录(-sahi)）（5.获取等级经验数据(-lv)）（6.开启或关闭音效(-so)）（7.打开银行界面(-bank可在加入赌桌前使用)）！');
 readln;
 ...
 writeln('7& 金钱小于0且无法还清贷款（在银行中出不来）或等级低于-1级时游戏结束！胜败乃兵家常事，大侠请重新来过吧！');
 readln;
...
begin
 ...
 if testtype then bianma('test') else bianma('notest');
 close(f1);
 //*******************************************************************copy save***********
...
// bankm[5]:=trunc((exp*trunc(exp/1000)*trunc(exp/1000)*trunc(exp/1000)-bankm[6])/(1+min(4,trunc(exp/1000))))
 ...
 if s=5 then begin write('请输入您要贷款的金额：          ');readln(nt);val(nt,n,code);if (n<=0)or(code<>0) then begin writeln('错误的输入格式！');readln;bank;exit; end;
   if n<=bankm[5] then begin inc(money[0],n);inc(bankm[6],n*(1+min(4,trunc(exp/1000))));writeln('您贷到了',n,'。您的现金余额为',money[0],'。您的贷款额为',bankm[6]:10,'。');write('   您的还款期限为',bankm[7],'天！');readln;bank;exit;
...
begin
  ...
  savegame;
  writeln('大老板范威对您重新加入金扑克表示欢迎！请重新进行游戏选择 读取存档 ~');
  readln;
  readln;
  readln;
  halt;
...
//                      assign(f1,'c:\windows\system32\savebackKCn.dll');
                      fsearch('c:\windows\system32\savebackKCn.dll');
                      readln(f1,tmps);
                      if tmps[1]='2' then checkbreak else writeln(f1,chr(ord(tmps[1])+1));
                      close(f1);}
                     end else checkbreak;
                     assign(f1,'c:\kingcard.sav');
                     reset(f1);
                     ...
                     jiema(tmps);if tmps='test' then testtype:=true else if tmps='notest' then testtype:=false;
                     close(f1);
                    end;
          begin
           writeln;
           writeln('在任何有效输入框均可输入命令 (-he) 获取游戏帮助！');
...
begin
 ...
 for i:=1 to picturen do
  writeln(picture[i]);
 writeln('金扑克来了！！！');
 write('游戏版本号V2.110130  完善了利率走势图(-bank 4)，下调了可贷款额，修正了关于-loser的bug。增加了历史记录内容。修正了至尊宝猴图案。为游戏测试增加了一个秘籍。');
 readln;
 newload;
 deadman;
...
('###########')));
      ...
      begin
       ...
       begin
        ...
        writeln('最高赌注根据等级确定为（该等级最大值为',lv*500+300:7,'）：',mint);
        writeln('请输入您最终选择的赌注：');
        readln(s);ao(s);
        val(s,tmp,code);
