program kingcard;{最新bug未修正：击败至少一个对手后-loser会判定为全部击败对手！股市K线图（1-75）！备份存档文件判断是否修改}
type
 cardtype=record
  a:char;
  b:string;
...
begin
 ...
 if s=4 then begin writeln('现在利率为',lilv,'%。您当前定期利息为',bankm[4],'。总取款所得为',bankm[1],'！');readln;bank;exit; end else
 ...
 s:string;
          ...
          begin
           ...
           if s='load' then
             ...
             time:=0;lilv:=0;lilvend:=0;
             fillchar(lilvlast,sizeof(lilvlast),0);
             rp:=100;
             savegame;
            end;
...
begin
 ...
 write('游戏版本号V1.091105.2253  修正了部分显示性错误和贷款中致命bug，下调了贷款期限50天之后的贷款利率，修正了满人-loser后依然计算天数的bug。');
 readln;
 newload;
 deadman;
