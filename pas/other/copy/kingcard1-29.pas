program kingcard;{最新bug未修正：击败至少一个对手后-loser会判定为全部击败对手！备份存档文件判断是否修改！(网络存盘有限读取次数)c:\windows\system32\savebackKC.dll胜利后继续旧存档游戏（人品不变）！战绩中N人赌桌（杀人数/总局数）history[11-12]}
type
 cardtype=record
  a:char;
  b:string;
...
begin
 ...
 writeln('       对手死亡率：每赌局死亡的对手/曾经总共进行的局数。总死亡率：最大值为500%');
 readln;
...
begin
 ...
 begin
  ...
  if tmpc[i]=0 then writeln('-':7)
   ...
   else writeln(round(tmpc[i]/tmp[6]*100):6,'%');
 end;
 write('胜利比率':12);
 for j:=2 to 5 do
 begin
  if tmp[j]=0 then write('-':15)
   ...
   else write(round((1-(history[j,1]/tmp[j]))*100):14,'%');
 end;
 if tmp[6]=0 then write('-':7)
  else write(round((1-tmpc[1]/tmp[6])*100):6,'%');
 writeln;
 //breakdown
 write('对手死亡率':12);
 for j:=2 to 5 do
 ...
 writeln((history[2,11]+history[3,11]+history[4,11]+history[5,11])*100/
...
begin
 ...
 begin
  ...
  if tmpc[i]=0 then writeln(f1,'-':7)
   ...
   else writeln(f1,round(tmpc[i]/tmp[6]*100):6,'%');
 end;
 write(f1,'胜利比率':12);
 for j:=2 to 5 do
 begin
  if tmp[j]=0 then write(f1,'-':15)
   ...
   else write(f1,round((1-(history[j,1]/tmp[j]))*100):14,'%');
 end;
 if tmp[6]=0 then write(f1,'-':7)
  else write(f1,round((1-tmpc[1]/tmp[6])*100):6,'%');
 writeln(f1);
 write(f1,'对手死亡率':12);
 for j:=2 to 5 do
 begin
 ...
 writeln(f1,(history[2,11]+history[3,11]+history[4,11]+history[5,11])*100/
         (history[2,12]+history[3,12]+history[4,12]+history[5,12]):6:1,'%');
 writeln(f1,'********************************************************************************');
 close(f1);
