program kingcard;{最新bug未修正：异地存盘有限读取次数c:\windows\system32\savebackKCn.dll。多天候存储=。=高赌注模式电脑选择赌注212，任何窗口输入命令}
type
 cardtype=record
  a:char;
  b:string;
...
var
 ...
 alive,players,host,playtype:integer;
...
begin
 ...
 for i:=1 to picturen do
  writeln(picture[i]);
 writeln('金扑克来了！！！');
 write('游戏版本号V2.110212  稍微上调了电脑的生存率。上调了一级赌注。增加了新的游戏模式。增加了胜利称号。缩短了高等级的还款期限。为游戏测试增加了一个秘籍。');
 readln;
 newload;
 deadman;
...
//******************************************************************************
       ...
       writeln(namep[h]:namek,'选择了赌注 ',mid,'元！');readln;
